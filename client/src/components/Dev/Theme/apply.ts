...
export function exportCss(state: LabState): string {
  return [
    ...
    ' *',
    ' * Paste this at the VERY END of client/src/style.css, at the top level.',
    ' * These are complete rules. Dropping them inside an existing html { } or',
    ' * .dark { } block nests them, and the stylesheet fails to build.',
    ' *',
    ' * Afterwards, hit Reset in the panel so the theme comes from this file',
